...
export class GameScene extends Phaser.Scene {
  ...
  setupBackground() {
    ...
    this.leftShadow = this.add.rectangle(tW + 6, GAME_HEIGHT / 2, 14, GAME_HEIGHT, 0x000000, 0.28);
    this.rightShadow = this.add.rectangle(GAME_WIDTH - tW - 6, GAME_HEIGHT / 2, 14, GAME_HEIGHT, 0x000000, 0.28);
  }
  ...
  setupCollisions() {
    this.physics.add.overlap(this.playerBullets, this.enemies, (bullet, enemy) => {
      ...
      if (enemy.hp <= 0) this.destroyTarget(enemy, 120, CONFIG.fx.impactParticles);
    });
  ...
  updatePlayerMovement() {
    const usingTouch = this.touchPointer?.isDown;
    ...
    if (usingTouch) {
      ...
      this.player.x = Phaser.Math.Linear(this.player.x, targetX, CONFIG.player.touchFollowLerp);
      this.player.y = Phaser.Math.Linear(this.player.y, targetY, CONFIG.player.touchFollowLerp);
